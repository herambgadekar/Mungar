...
class CharacterMotorMovement {
	...
	var gravity : float = 10.0;//20?
	var maxFallSpeed : float = 20.0;
	
	
	///wei zhang modified
	var fallonthetree: float=0.1;
...
var thetree:GameObject =null;
var onthetree:boolean =true;
var positiononthetree :Vector3 =Vector3.zero;
...
private function ApplyInputVelocityChange (velocity : Vector3) {	
	...
	if (grounded && TooSteep()) {
		...
		{    // Wei zhang
			//no input because the desired velocity is changed modified by weizhang
		  //desiredVelocity = GetDesiredHorizontalVelocity();//just for debug , orgiin
			desiredVelocity=this.movement.velocity;
		}
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	//Debug.Log("input velocity:"+velocity);
	...
	{	
		 ...
		 if(velocity.y<(0-movement.fallonthetree))
		 	{
		 	 velocity.y=(0-movement.fallonthetree);	 	 
		 	}
		//velocity.y=movement.velocity.y+movement.gravity*0.9*Time.deltaTime;
	}
...
function OnControllerColliderHit (hit : ControllerColliderHit) {
   	Debug.Log("hit terrain"+hit.gameObject.name);
    if(hit.gameObject.name.ToLower().Contains("terrain"))
				{
					this.grounded=true;
	...
	if (hit.normal.y > 0 && hit.normal.y > groundNormal.y && hit.moveDirection.y < 0) {
		...
		// else modified by wei zhang, origin no else
		
		 var  hittree :boolean=hit.gameObject.name.ToLower().Contains("pine");
		if(hittree)
		{
			Debug.Log(hit.point);
			 
			var x:float=tr.position.x;
			var z:float=tr.position.z;
			Debug.Log(x+":"+z);
			this.onthetree=true;
			this.thetree=hit.gameObject;
			//thetree.collider.isTrigger=true;
...
//Debug.Log(this.groundNormal.y.ToString("E5"));
